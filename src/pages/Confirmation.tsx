    ...
    const fetchBankAccountDetails = () => {
      if (isFrenchShop) {
        ...
        if (gasolioCasaAccount) {
          ...
          console.log('Confirmation - Using hardcoded Italian account holder: Gasolio Veloce');
        }
      }
    };
              ...
              <div className="lg:col-span-2 space-y-6">
                ...
                >
                  ...
                  <h1 className="text-3xl font-bold text-gray-900 mb-4">
                    {isItalianShop ? 'Ordine confermato!' : (isFrenchShop ? 'Commande confirmée !' : 'Vielen Dank für Ihre Bestellung!')}
                  </h1>
                  ...
                  <div className="bg-red-50 border border-red-200 rounded-lg p-4 inline-block">
                    <div className="text-sm text-red-600 font-medium">
                      {isItalianShop ? 'Il tuo numero d\'ordine' : (isFrenchShop ? 'Votre numéro de commande' : 'Ihre Bestellnummer')}
                    </div>
                  ...
                  {(isFrenchShop || isItalianShop) && orderData.customerEmail && (
                    <div className="mt-6 p-4 bg-green-50 border border-green-200 rounded-lg">
                      ...
                      <p className="text-green-600 text-sm mt-2">
                        {isItalianShop 
                          ? <>La tua fattura è stata automaticamente inviata a <strong>{orderData.customerEmail}</strong> con i dati bancari.</>
                ...
                >
                  <h3 className="text-xl font-bold text-gray-900 mb-4">
                    {isItalianShop ? 'Il tuo fornitore' : (isFrenchShop ? 'Votre fournisseur' : 'Ihr Lieferant')}
                  </h3>
                ...
                >
                  <div className="flex items-center mb-6">
                    ...
                    <div>
                      <h3 className="text-xl font-bold text-gray-900">
                        {isItalianShop ? 'Istruzioni di pagamento' : (isFrenchShop ? 'Instructions de paiement' : 'Zahlungshinweise')}
                      </h3>
                      <p className="text-gray-600">
                        {isItalianShop ? 'Come pagare il tuo ordine' : (isFrenchShop ? 'Comment payer votre commande' : 'So zahlen Sie Ihre Bestellung')}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="space-y-4">
                    <div className="bg-blue-50 rounded-lg p-4">
                      <h4 className="font-semibold text-blue-900 mb-3">
                        {isItalianShop ? 'Prossimi passi' : (isFrenchShop ? 'Prochaines étapes' : 'Nächste Schritte')}
                      </h4>
                ...
                >
                  <div className="flex items-center mb-6">
                    ...
                    <div>
                      <h3 className="text-xl font-bold text-gray-900">
                        {isItalianShop ? 'Informazioni sulla consegna' : (isFrenchShop ? 'Informations de livraison' : 'Lieferinformationen')}
                      </h3>
                      <p className="text-gray-600">
                        {isItalianShop ? 'Dettagli importanti sulla tua consegna' : (isFrenchShop ? 'Détails importants sur votre livraison' : 'Wichtige Details zu Ihrer Lieferung')}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="bg-orange-50 rounded-lg p-4">
                      ...
                      <div className="text-orange-800 font-bold">
                        {isItalianShop ? '3-5 giorni lavorativi' : (isFrenchShop ? '2-4 jours ouvrables' : orderData.deliveryDate)}
                      </div>
                    ...
                    <div className="bg-gray-50 rounded-lg p-4">
                      <div className="font-semibold text-gray-900 mb-2">
                        {isItalianShop ? 'Indirizzo di consegna' : (isFrenchShop ? 'Adresse de livraison' : 'Lieferadresse')}
                      </div>
                  ...
                  <div className="mt-4 p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
                    <h4 className="font-semibold text-yellow-800 mb-2">
                      📞 {isItalianShop ? 'Nota importante sulla consegna' : (isFrenchShop ? 'Remarque importante sur la livraison' : 'Wichtiger Hinweis zur Lieferung')}
                    </h4>
                ...
                >
                  <h3 className="text-xl font-bold text-gray-900 mb-4">
                    {isItalianShop ? 'Domande sul tuo ordine?' : (isFrenchShop ? 'Questions sur votre commande?' : 'Fragen zu Ihrer Bestellung?')}
                  </h3>
                    ...
                    <div className="flex items-center space-x-3 p-3 bg-gray-50 rounded-lg">
                      ...
                      <div>
                        ...
                        <div className="text-gray-600 text-sm">
                          {isItalianShop ? 'servizio@gasoliocasa.com' : (isFrenchShop ? 'service@fioul-rapide.fr' : 'service@heizoeldirekt.de')}
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="mt-4 text-center">
                    ...
                    >
                      {isItalianShop ? 'Nuovo ordine' : (isFrenchShop ? 'Nouvelle commande' : 'Neue Bestellung aufgeben')}
                    </Button>
                  </div>
                </motion.div>
              </div>
                ...
                <OrderSummary 
                  orderData={{
                    product: {
                      ...
                      description: isItalianShop ? 'Gasolio di qualità secondo norma DIN 51603-1' : (isFrenchShop ? 'Fioul de qualité selon norme DIN 51603-1' : 'Qualitäts-Heizöl nach DIN 51603-1')
                    },

...
export const useOrders = () => {
  ...
  // Get the "Gasolio Casa" bank account ID for Italian orders
  const getGasolioCasaBankAccountId = () => {
    const gasolioCasaAccount = bankAccounts.find(
      account => account.system_name === 'Gasolio Casa'
    );
  ...
  const createOrder = async (orderData: Omit<OrderInsert, 'order_number' | 'request_id'>) => {
    try {
      ...
      // For Italian shop orders, automatically assign Gasolio Casa bank account and shop
      if (isItalianShop) {
        ...
        if (gasolioCasaBankAccountId) {
          ...
          console.log('Automatically assigned Gasolio Casa bank account for Italian shop order');
        }
      ...
      // Handle post-order processing based on shop type
      ...
      } else if (isItalianShop && data.bank_account_id && data.shop_id) {
        // For Italian shop orders, automatically generate and send invoice (no order confirmation email)
        try {
          ...
          toast({
            title: 'Ordine Confermato',
            description: 'Ordine creato con successo. La fattura verrà inviata a breve.',
          });
        }
      } else {
        // For non-French and non-Italian shops, show normal success message
        toast({
          title: 'Erfolg',
